...
def call_model_headless(
    ...
    try:
        ...
        return {"error": str(e), "exit_code": -1, "error_type": type(e).__name__}
...
def _read_status_secure(path: Path) -> Optional[dict]:
    ...
    try:
        ...
        fd = os.open(str(path), os.O_RDONLY | nof)
    ...
    try:
        ...
        if ckpt_ctx:
            ...
            if len(candidate) > MAX_TOTAL_CHARS:
                keep_tail = base_prompt[-80000:] if len(base_prompt) > 80000 else base_prompt
                for crit_marker in ("JOB-TO-THE-END RULE (HIGHEST PRIORITY FOR THIS INVOCATION):", "MANDATORY FINAL OUTPUT FORMAT", "=== DELEGATION SUMMARY ==="):
                    idx = base_prompt.rfind(crit_marker)
                    if idx != -1 and len(base_prompt) - idx < 150000:
                        keep_tail = base_prompt[idx:]
                        break
...
def _write_prompt_audit(target_dir: str, run_id: str, label: str, prompt: str, provenance: Optional[dict] = None) -> Optional[Path]:
    ...
    try:
        ...
        return prompt_path
    except (OSError, RuntimeError, ValueError, TypeError, json.JSONDecodeError, re.error):
        return None
...
def _wait_for_background_completion(
    ...
    if stop_event is not None and stop_event.is_set():
        status_manager.update(last_poll_error="stop_event set")
        return {"error": "stopped", "exit_code": -1, "stopped": True, "run_id": run_id, "run_name": run_name}
    ...
    try:
        ...
        try:
            status_manager.update(last_poll_error="audit_fail:probe001")
        except (OSError, RuntimeError, ValueError, TypeError, json.JSONDecodeError):
            pass
    ...
    try:
        ...
        if not first_result.get("timed_out"):
            ...
            return first_result
    except (OSError, subprocess.SubprocessError, RuntimeError, ValueError, TypeError, json.JSONDecodeError):
        pass  # Fall through to normal polling loop

    status_manager.record_poll(time.time() - start_time)
    ...
    while True:
        ...
        try:
            ...
            try:
                status_manager.update(last_poll_error=f"audit_fail:probe{probe_count}")
            except (OSError, RuntimeError, ValueError, TypeError, json.JSONDecodeError):
                pass
...
def load_checkpoint_context(target_dir: str) -> str:
    ...
    HYGIENE (Dialogue.md P0 item): Strict JSON-only + narrative stripping makes
    "no raw narrative / contamination from PROGRESS.json etc." automatic and
    non-negotiable. See candidates list and _strip_narrative below.
    ...
    # Strict JSON-only for contamination prevention (per Dialogue.md "Harness Next Tasks Perspective"
    # 2026-06-01, the P0 "Checkpoint and state injection hygiene" item that was blocking other work
    # and surfaced as "gone back to crashing" after recent dogfood runs with the modern pattern).
    #
    # - Only JSON candidates (removed all .md and free-form paths that could carry raw narrative).
    # - Aggressive per-string sanitization below (strips instruction markers, old task language, etc.).
    # - Combined with the UNTRUSTED wrapper + ownership/size caps + audit tying, this makes
    #   "strict JSON whitelist only, no raw narrative" automatic and non-negotiable inside the harness.
    # - Every successful injection is expected to be accompanied by a .cdh-prompts/ audit entry
    #   at the call sites (launch, probes, resume, remediation).
    candidates = [
        target / "PROGRESS.json",
        target / "TASK_PROGRESS.json",
        target / "checkpoint.json",
        target / ".cdh-progress.json",  # harness-controlled name if we ever write sanitized copies
    ]
            ...
            try:
                ...
                if content:
                    ...
                    if isinstance(parsed, dict):
                        sanitized = {}
                        # Aggressive narrative stripping for strings (makes "strict JSON whitelist only, no raw narrative" automatic).
                        # Any value containing instruction-like content, markers, or old task language is reduced to safe summary.
                        def _strip_narrative(s: str) -> str:
                            if not isinstance(s, str):
                                return str(s)[:200]
                            # Remove obvious instruction / marker / code blocks that could carry contamination
                            bad_markers = ("You are ", "You will ", "=== ", "TASK:", "CRITICAL:", "INSTRUCTIONS:", "```", "PROMPT:", "Do not ")
                            lines = []
                            for line in s.splitlines():
                                l = line.strip()
                                if any(m in l for m in bad_markers):
                                    continue
                                if len(l) > 300:  # truncate over-long narrative lines
                                    l = l[:300] + " [truncated]"
                                lines.append(l)
                            out = " ".join(lines).strip()
                            return out[:500] if out else "[sanitized]"
                        ...
                        for key in allowed_keys:
                            ...
                            if isinstance(value, (str, int, float, bool)) or value is None:
                                if isinstance(value, str):
                                    sanitized[key] = _strip_narrative(value)
                                else:
                                    sanitized[key] = value
                                ...
                                for item in value[:25]:
                                    if isinstance(item, (str, int, float, bool)):
                                        if isinstance(item, str):
                                            cleaned.append(_strip_narrative(item))
                                        else:
                                            cleaned.append(item)
...
def acquire_target_lock(target_name: str, lock_base_dir: Optional[Path] = None):
    ...
    if lock_dir.is_symlink():
        raise RuntimeError(f"Lock directory is a symlink (refusing to use untrusted path): {lock_dir}")
    lock_dir.mkdir(parents=True, exist_ok=True, mode=0o700)
    lock_path = lock_dir / f"{target_name}.lock"

    try:
        if lock_path.is_symlink():
            raise RuntimeError(f"Lock file is a symlink (refusing to use untrusted path): {lock_path}")
        tmp_lock = lock_path.with_suffix(".lock.tmp")
        try:
            if tmp_lock.exists():
                tmp_lock.unlink()
        except (OSError, RuntimeError, ValueError, TypeError):
        ...
        if not lock_path.exists():
            fd = os.open(str(tmp_lock), os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
            os.close(fd)
            try:
                d_fd = os.open(str(tmp_lock.parent), os.O_RDONLY)
                try:
                    os.fsync(d_fd)
                finally:
                    os.close(d_fd)
                ...
                pass
            os.replace(str(tmp_lock), str(lock_path))
            try:
                d_fd = os.open(str(lock_path.parent), os.O_RDONLY)
                try:
                    os.fsync(d_fd)
                finally:
                    os.close(d_fd)
                ...
                pass
        nof = getattr(os, "O_NOFOLLOW", 0)
        fd = os.open(str(lock_path), os.O_RDWR | nof, 0o600)
        lock_file = os.fdopen(fd, "w+")
    except OSError:
        raise RuntimeError(f"refusing to open lock path that is or became a symlink under untrusted target_dir: {lock_path}") from None
    try:
...
def main():
    ...
    if getattr(args, "task_file", None):
        try:
            ...
            if tf.is_symlink():
                parser.error(f"Refusing to read --task-file that is a symlink: {args.task_file}")
            nof = getattr(os, "O_NOFOLLOW", 0)
            fd = os.open(args.task_file, os.O_RDONLY | nof)
            try:
                st = os.fstat(fd)
                if st.st_size > 2000000:
                    os.close(fd)
                    parser.error(f"--task-file too large (>2MB): {args.task_file}")
                with os.fdopen(fd, "r", encoding="utf-8") as f:
                    file_task = f.read()
            except Exception:
                try:
        ...
        try:
            ...
            try:
                ...
                if sm_early.load(require_owner_and_secure=False):
                    sm_early.update(invocation=" ".join(shlex.quote(a) for a in sys.argv))
            except (OSError, RuntimeError, ValueError, TypeError, json.JSONDecodeError):
                pass
    ...
    if args.resume:
        ...
        try:
            ...
            ec = result.get("exit_code", 0) if isinstance(result, dict) else 0
            sys.exit(0 if ec in (0, None) else 1)
            ...
            try:
                ...
                if entry["state"] in ("waiting", "launched", "running"):
                    ...
                    try:
                        ...
                        try:
                            esm = StatusManager(sf)
                            if esm.load(require_owner_and_secure=False):
                                esm.update(last_error=f"looks_dead_fail:{type(e).__name__}")
                        except Exception:
                            pass
                    active.append(entry)
                else:
        ...
        for sf in Path(target_dir).glob(".cdh-run-*.status"):
            try:
                ...
                try:
                    esm = StatusManager(sf)
                    if esm.load(require_owner_and_secure=False):
                        esm.update(last_reap_error=f"reap_fail:{type(e).__name__}:{str(e)[:100]}")
                except Exception:
                    pass
    ...
    # Gentle warning for non-git directories (rich diffs, previews, and .patch file generation depend on git)
        ...
        for sf in list(Path(target_dir).glob(".cdh-run-*.status")):
            try:
                ...
                try:
                    esm = StatusManager(sf)
                    if esm.load(require_owner_and_secure=False):
                        esm.update(last_reap_error=f"auto_reap_fail:{type(e).__name__}:{str(e)[:100]}")
                except Exception:
                    pass
        if reaped > 0:
    ...
    if getattr(args, "iterations", None) is not None:
        # --iterations 2 means 1 original + 1 attack pass = remediation_max_passes=1
    ...
    if not getattr(args, "skip_normalization", False):
        try:
            ...
            if verbosity >= 1:
                print(f"[cdh] Warning: intake gate unavailable ({intake_exc}). Passing raw prompt through.")
            intake_result = None
    ...
    # -------------------------------------------------------------------------
    hygiene_suffix = ""
    if not getattr(args, "no_hygiene", False):
        try:
    ...
    try:
        ...
        if stale_sentinel.exists() and not stale_sentinel.is_symlink():
    ...
    if not getattr(args, "quiet", False):
        print(f"[cdh] Starting delegation at {datetime.now(timezone.utc).isoformat()}")
    ...
    if args.auto_remediate and args.remediation_max_passes > 0:
        ...
        # Always record the remediation decision for auditability (even if we skip the actual pass2)
        clean_result.setdefault("metadata", {})
        clean_result["metadata"]["auto_remediate_requested"] = True
        clean_result["metadata"]["remediation_decision"] = {
            "should_remediate": should_remediate,
            "reason": remediation_reason,
            "mode": args.remediation_mode,
            "max_passes": args.remediation_max_passes,
        }
        if should_remediate and args.remediation_mode == "targeted-inversion":
            ...
            if audit_pass2:
                ...
                try:
                    audit_dir = str(Path(target_dir) / ".cdh-prompts")
                    # Use locals() (correct snapshot) instead of fragile dir() hack.
                    # Addressing Dialogue.md "Checkpoint and state injection hygiene" P0 item
                    # (and the post-dogfood crashing regression).
                    # The launch_status_file is always created for delegation paths that reach
                    # auto-remediation + output artifacts (post-wait or direct). This removes
                    # a source of intermittent scope/rebinding crashes seen after long dogfood runs
                    # that exercised wait + --auto-remediate.
                    sm = StatusManager(launch_status_file) if 'launch_status_file' in locals() else None
                    if sm and sm.load():
