...
- The harness itself now actively helps escape the very constrained environments it is often developed inside, so that a strong implementation model can use it without constant outer intervention or leaving broken live targets.

These anti-patterns (drift, prompt softening, manual repair passes on dogfood targets after harness deaths) are closed. The tree is now in a state worth checking in and checking out.
