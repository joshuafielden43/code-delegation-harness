...
Mission (v4 scoped pass)
Run a tightly controlled, narrow incremental normalization pass and produce **review-ready artifacts only**.

Scope for this validation pass (keep extremely narrow)
- Only the `yt -> youtube` (or equivalent YouTube-related) normalization.
- Do **not** do a broad tag-grooming sweep. Stay focused on this single high-signal decision.
...
- Updated decisions document (`tag-normalization-decisions-v4.md` or equivalent)
  - Statuses: Proposed / Ready for Review / Deferred (+ explicit reason)
- `PROGRESS.json` checkpoints at the required points

Canonical casing rule (for this tiny slice)
- Use one consistent canonical form only: `youtube` as a lowercase tag slug.
- Do not mix `YouTube` / `youtube` / `Youtube` in the same tag list for this run.
- If a specific file in the vault forces title-case by local convention, defer the change rather than introduce mixed casing.
...
Recommended PROGRESS.json shape
{
  "completed": ["..."],
  "current_phase": "...",
  "next_steps": ["..."],
...
Strict Patch Generation Rules (v4 tightening — this is the core of this iteration)
...
1. Discover prior outputs and decisions doc paths
2. Write initial PROGRESS.json
3. Create a tight, narrow plan for the `yt -> youtube` slice only
4. Perform discovery against actual vault snapshot
5. Attempt patch generation **only** for files that exist and match
6. Validate every candidate patch against the snapshot
7. Update decisions doc with honest statuses
8. Run all quality gates
9. Emit final summary with required markers

Deliver clean, reviewable, high-signal artifacts only. This pass is deliberately narrow so we can validate the new live-vault patch discipline before widening scope.

Start by locating the previous normalizer outputs and the actual vault structure. Do not assume file locations or structures.

---

## Harness Grooming Notes Support (produces even higher-signal reviewer feedback)

When writing your required PROGRESS.json checkpoints, include these optional but high-value fields so the outer harness can surface **first-class structured notes** in the final human report (directly enabling the quality of feedback Honey produced on v4):

Recommended rich fields (use what fits the work):

```json
{
  "validation_status": "PASS_GATES | PARTIAL (0 real patches — all candidates already canonical or deferred)",
  "cluster_evidence": {
    "youtube": {
      "reviewed": 18,
      "already_canonical": 1,
      "deferred": 0,
      "rationale": "No 'yt' frontmatter tags existed in live vault snapshot; independent parser cross-check passed"
    }
  },
  "real_target_evidence": "Every candidate validated against temp snapshot of actual vault file; 0 synthetics promoted",
  "canonical_rules": "youtube lowercase only for this slice",
  "decisions": { "04 Research/AI/YouTube.md": "already exact canonical — no patch needed" },
  "gotchas": ["..."],
  "open_issues": ["..."]
}
```

The harness will lift `cluster_evidence`, `validation_status`, `real_target_evidence`, `decisions`, `canonical_rules` etc. into a prominent **♻️ Grooming / Normalization Notes** section + **Run Intent** callout in the `.report.md`.

This is the mechanism for turning good runs into *Honey-grade review notes* automatically. Use it on all future grooming/normalization dogfood passes.
